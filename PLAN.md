...
- **训练计划问卷（新增）**：生成训练计划前弹出 7 题问卷：
  ...
  - 训练强度（温和/中等/高强度）
- Prompt 包含：用户个人信息（性别、年龄、体重、身高、目标）+ 7 题问卷答案
...
- **无预设计划种子数据**：不再预置 `wp-1` 新手计划，用户需手动生成
- 数据不足时（缺性别/年龄/体重/身高）返回提示，不调用 AI
- **饮食计划 AI 自行计算热量**：传给 AI Mifflin-St Jeor 公式和活动系数，AI 自己算 dailyCalories
- **活动量选择**：生成饮食计划前选 5 级活动量（久坐不动~运动员），影响系数和 AI 输出
- **主页热量目标来源**：从 AI 饮食计划读取 dailyCalories，不再使用服务端公式单独计算
