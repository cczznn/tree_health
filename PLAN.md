...
### 目标
基于饮食、计划和身体数据生成受约束、可解释、可回退的建议。T10 已在独立 worktree `E:/6/ai/last-t10` 中完成，采用规则驱动的结构化推荐生成，不引入自由聊天式输出。
...
- 数据不足时返回通用建议
- 不输出高风险医疗结论
- 推荐结果包含可解释元数据（如来源与降级原因）

### 验证步骤
1. 先写失败测试：给定不同输入时输出类别正确
2. 先写失败测试：数据不足时会触发降级模板
3. 先写失败测试：建议内容包含可解释字段
4. 先写失败测试：推荐结果可写入推荐仓储
5. 实现最少推荐生成逻辑
6. 重构提示词构造与规则层

### 结果记录
- 在 `E:/6/ai/last-t10` worktree 中完成实现
- 新增 `RecommendationService`
- 补齐推荐仓储与推荐结果结构，支持结构化输出与 fallback 输出
- 先写 `tests/recommendations/recommendation-service.test.ts`，覆盖结构化推荐、数据不足降级与可解释元数据
- 按饮食、训练计划和身体数据生成规则化推荐，避免自由聊天式输出
- `npm run typecheck` 与 `npm test` 均通过
- 对应 commit hash：`TBD`
